...
benchmark_dir = "../../ARCH-COMP2024/benchmarks/Benchmark9-Tora/"
...
lirpa_model = BoundedModule(model_ori, torch.zeros(1, *input_shape[1:]), device=device, bound_opts={'activation_bound_option': 'same-slope', 'conv_mode': 'matrix'})
...
def CROWN_reach(input_lb, input_ub):
    ...
    coefficients["T"] = A['lA'].view(output_T_shape).cpu().tolist()
    coefficients["u_min"] = A['lbias'].view(output_c_shape).cpu().tolist()
    coefficients["u_max"] = A['ubias'].view(output_c_shape).cpu().tolist()
    return coefficients
