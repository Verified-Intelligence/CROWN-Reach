...
class BoundTanh(BoundOptimizableActivation):
    ...
    def bound_relax_impl_same_slope(self, x, func, dfunc):
        ...
        k_direct[k_direct < 1e-4] = 0   # When the slope is flat enough

        mask_direct_lower = k_direct <= dfunc(lower)
        mask_direct_upper = k_direct <= dfunc(upper)
        # print(mask_direct_lower)
        # print(mask_direct_upper)
        ...
        # mask_right_out_of_range = k * (upper - d_same_slope) + y_d_same_slope > func(torch.tensor(100))
        # mask_left_out_of_range = k * (lower + d_same_slope) + y_d_same_slope_opposite < func(torch.tensor(-100))
        # mask_out_of_range = mask_right_out_of_range * mask_left_out_of_range
        # self.add_linear_relaxation(
        #     mask=mask_both*(~mask_out_of_range), type='upper', k=k, x0=d_same_slope, y0=y_d_same_slope
        # )
        # self.add_linear_relaxation(
        #     mask=mask_both*(~mask_out_of_range), type='lower', k=k, x0=-d_same_slope, y0=y_d_same_slope_opposite
        # )
        # self.add_linear_relaxation(
        #     mask=mask_both*mask_out_of_range, type='upper', k=torch.zeros_like(k), x0=upper, y0=y_u
        # )
        # self.add_linear_relaxation(
        #     mask=mask_both*mask_out_of_range, type='lower', k=torch.zeros_like(k), x0=lower, y0=y_l
        # )
